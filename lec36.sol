...
/*
    call : 로우레벨 함수
    1. 송급하기
    2. 외부 스마트 컨트렉트 함수 부르기
    3. 가변적인 gas
    4. 이스탄불 하드포크, 2019년 12월 이후 gas 가격 상승에 따른, call 사용 권장 / send transfer = 2300gas
...
contract lec36_caller {
    event calledFunction(bool _success, bytes _output);

    // 1. 송급하기
    function transferEther(address payable _to) public payable {
        (bool success,) = _to.call{value:msg.value}("");
        require(success, "failed to transfer either");
    }

    // 2. 외부 스마트 컨트렌트 함수 부르기
    ...
    function callMethodNothing(address _contractAddr) public {
